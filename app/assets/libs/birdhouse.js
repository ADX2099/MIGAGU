...
function BirdHouse(params) {
	...
	function get_request_token(callback) {
		...
		//Ti.API.info("Params en getRequestTokem: " + JSON.stringify(params));
		//alert("Si llegue hasta aqui man::....");
	...
	function get_request_verifier(callback) {
		...
		Ti.API.info("que trae url la webView..........: " + url);
		...
		/*
		 * SECCION IPAD y IPHONE
		 */
		if (Alloy.platform.isiPad) {
			...
			var b = Titanium.UI.createButton({
				...
				height : 36,
				width : 36,
				backgroundImage:"/btn-cerrar.png"
			});
	...
	function load_access_token() {
		...
		Ti.API.info("que trae file3load: " + file + " FileJson3: " + JSON.stringify(file));
		if (!file.exists()) {
			return false;
		}
	...
	function save_access_token() {
		...
		var file = Ti.Filesystem.getFile(Ti.Filesystem.applicationDataDirectory, 'twitter.config');
		Ti.API.info("que trae file4save: " + file + " FileJson4: " + JSON.stringify(file));
		...
		if (!authorized && (typeof(auth)=='undefined' || auth===true)) {
			authorize(function(retval){
				if (!retval) {
					...
					Ti.API.info("Que trae url en la funcion API: " + url + " PArams: " + JSON.stringify(params));
	...
	function tweet(text,callback) {
		...
		obj.mytweet = text;
		if (authorized === false) {
			//alert("AL PARECER NO ESTOY AUTORIZADO");
			...
			btnTW.addEventListener('click',function() {
				...
				send_tweet("status="+encodeURIComponent(tweet.value),function(retval){
	...
	function send_tweet(params,callback) {
		Ti.API.info("::los params" + params);
		api('https://api.twitter.com/1.1/statuses/update.json','POST',params,function(resp){
			if (resp!=false) {
				if (typeof(callback)=='function') {
					callback(true);
				}
	...
	function get_tweets(params,callback) {
		...
		Ti.API.info("Que trae params en get_tweets:  " + JSON.stringify(params));
	...
	function authorize(callback) {
		...
		if (!authorized) {
			Ti.API.info("A BUSCAR AUTORIZACIONA");
