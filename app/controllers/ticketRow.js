...
/*
 * FUNCION QUE VERIFICA EL MOTIVO Y REALIZA LOS CAMBIOS APROPIADOS EN LA INTERFAZ EN CASO DE SER NECESARIO
 */
function verificar_motivo(){
	var motivo = JSON.stringify(fila);
	var hijos = fila.getChildren();
	var hijosCad = JSON.stringify(hijos);
	
	Ti.API.info("EQUE VALOR TIENE " + hijos);
	Ti.API.info("MIS HIJOS VALOR TIENESTRIN " + hijosCad);
	var motivo = hijos[3].text;
	Ti.API.info("LOS VALORES SON::. " + motivo );
...
/*
 * Funcion encargada de mandar el numero de ticket y la bandera prendida de que sera favorito
 */
	...
	if (Alloy.network.isAvailable()) {
		...
		Alloy.network.callService('apexrest/AMFavorito', params, function(response) {
			...
			if (response.Error == "false") {
				...
				$.favorito.image = "/images/star.png";
				$.favorito.block = true;
				var alert = Ti.UI.createAlertDialog({
					title : "Atención",
					message : "Tu favorito se ha agregado",
					buttonNames : ["Ok"]
				});
				alert.show();
			} else {
				var alert = Ti.UI.createAlertDialog({
					title : "Atención",
					message : "No fue posible agregar tu Ruta a favorito" + response.ErrorMsg,
					buttonNames : ["Ok"]
				});
				alert.show();
...
function setHistorial(param){
	if (Alloy.network.isAvailable()) {
		...
		Alloy.network.callService('apexrest/AMFavorito', params, function(response) {
			...
			if (response.Error == "false") {
				//Ti.API.info("RESPONDIOS");
				$.historial.image = "/images/history_on.png";
				$.favorito.block = true;
				var alert = Ti.UI.createAlertDialog({
					title : "Atención",
					message : "Tu Reporte se ha agregado a historial",
					buttonNames : ["Ok"]
				});
				alert.show();
			} else {
				var alert = Ti.UI.createAlertDialog({
					title : "Atención",
					message : "No fue posible agregar tu Reporte a historial" + response.ErrorMsg,
					buttonNames : ["Ok"]
				});
				alert.show();
   	...
   	switch(seleccion.id){
   		case "favorito":
   			var alert = Ti.UI.createAlertDialog({
				...
				title : "Atención!",
				message : "¿Deseas agregar esta ruta a tu lista de favoritos?",
				buttonNames : ["Si", "No"],
				cancel : 1
			});
   		...
   		case "eliminar":
   			var alert = Ti.UI.createAlertDialog({
				...
				title : "Atención!",
				message : "¿Deseas eliminar este ticket de tu listado ?",
				buttonNames : ["Si", "No"],
				cancel : 1
			});
   		...
   		case "historial":
   			var alert = Ti.UI.createAlertDialog({
				...
				title : "Atención!",
				message : "¿Deseas agregar esta ticket a tu historial ?",
				buttonNames : ["Si", "No"],
				cancel : 1
			});
